...
def verifier_authentification():
    ...
    if "mlb_authentifie" not in st.session_state:
        st.session_state.mlb_authentifie = False

    if st.session_state.mlb_authentifie:
        return

    st.markdown("## 🔒 Accès Restreint")
    with st.form("mlb_login_form", clear_on_submit=False):
        mdp = st.text_input("Mot de passe :", type="password")
        if st.form_submit_button("Connexion"):
            if mdp == mot_de_passe_attendu:
                st.session_state.mlb_authentifie = True
                st.rerun()
            else:
